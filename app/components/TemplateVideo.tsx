...
const TemplateVideo: React.FC<TemplateVideoProps> = ({
  ...
  const processVideo = useCallback(async () => {
    ...
    try {
      ...
      const paddingCommand = `-i "${selectedVideo.uri}" -vf "pad=iw:ih+${videoCaptionHeight}:0:0:black" -pix_fmt yuv420p -b:v 3M -maxrate 4M -bufsize 8M -c:a copy "${tempVideoPath}"`;
      ...
      const overlayCommand = `-i "${tempVideoPath}" -i "${redBackgroundPath}" -filter_complex "[0:v][1:v]overlay=0:H-h" -pix_fmt yuv420p -b:v 3M -maxrate 4M -bufsize 8M -c:a copy "${tempWithRedPath}"`;
      ...
      const finalCommand = `-i "${tempWithRedPath}" -i "${captionResizedPath}" -filter_complex "[0:v][1:v]overlay=0:H-h,scale=trunc(iw/2)*2:trunc(ih/2)*2" -pix_fmt yuv420p -b:v 3M -maxrate 4M -bufsize 8M -c:a copy -map_metadata -1 "${outputPath}"`;
